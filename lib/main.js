...
function Presentation() { // eslint-disable-line stdlib/no-redeclare
	...
	if ( nargs === 1 ) {
		...
		text = arguments[ 0 ];
		if ( !isString( text ) ) {
			throw new TypeError( 'invalid argument. Presentation text argument must be a string. Value: `' + text + '`.' );
		}
		repl = arguments[ 1 ];
		options = arguments[ 2 ];
		err = validate( opts, options );
	...
	if ( constructorName( repl ) !== 'REPL' ) {
		throw new TypeError( 'invalid argument. REPL argument must be a REPL instance. Value: `' + repl + '`.' );
	}
	self = this;
